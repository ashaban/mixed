  ...
  app.get('/syncImmunizationCoverage', (req, res) => {
    ...
    mixin.prepareDataSync(parameters, (facilities, rows) => {
      ...
      async.eachSeries(facilities, (facility, nxtFacility) => {
        if(facility.vimsFacilityId != '19976' && facility.vimsFacilityId != '19978') {
          return nxtFacility()
        } else {
          facility.periodId = "237563"
          facility.periodName = "January 2020"
        }
            ...
            if (facData.length > 0) {
              async.parallel({
                  coldChainSync: callback => {
                    vims.populateColdChainLineItem(facData, facility, updatedLineItems, orchestrations, () => {
