...
const createESIndex = (name, IDFields, callback) => {
  ...
  let url = URI(config.elastic.server).segment(name).toString()
  axios({
    method: "head",
    url,
    auth: {
            ...
            async.whilst(
              ...
              () => {
                ...
                async.eachSeries(resourceData, (data, next) => {
                  ...
                  let record = {}
                  if (!Array.isArray(orderedResource.element)) {
                    let displayData = fhir.evaluate(data.resource, orderedResource.element)
                    if (Array.isArray(displayData)) {
                      record[orderedResource.element] = displayData.pop()
                    } else {
                      record[orderedResource.element] = displayData
                    }
                  } else {
                    for (let element of orderedResource.element) {
                      let displayData = fhir.evaluate(data.resource, element)
                      if (Array.isArray(displayData)) {
                        record[element] = displayData.pop()
                      } else {
                        record[element] = displayData
                      }
                    }
                  }
                  record[orderedResource.name] = data.resource.id
                  let match = {}
                  ...
                  let url = URI(config.elastic.server).segment(reportDetails.name).segment('_update_by_query').toString()
                  let body = {
                    "script": {
                      "lang": "painless",
                      "source": ctx,
                  ...
                  axios({
                    ...
                    if (response.data.updated == 0 && !orderedResource.hasOwnProperty('linkElement')) {
                      ...
                      let url = URI(config.elastic.server).segment(reportDetails.name).segment('_doc').toString()
                      axios({
                        method: 'post',
                        url,
                        data: record,
